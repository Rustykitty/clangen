    ...
    for event in possible_events:
        ...
        if event.injury:
            ...
            if cat.status in INJURY_DISTRIBUTION:
                minor_chance = INJURY_DISTRIBUTION[cat.status][
                    "minor"
                ]
                major_chance = INJURY_DISTRIBUTION[cat.status][
                    "major"
                ]
                severe_chance = INJURY_DISTRIBUTION[cat.status][
                    "severe"
                ]
...
def possible_ongoing_events(event_type=None, specific_event=None):
    ...
    if game.clan.biome not in game.clan.BIOME_TYPES:
        ...
        if not specific_event:
            event_list.extend(
                generate_ongoing_events(event_type, biome)
            )
            """event_list.extend(
...
def possible_death_reactions(family_relation, rel_value, trait, body_status):
    ...
    events = get_death_reaction_dicts("general", rel_value)
    ...
    if family_relation != "general" and rel_value != "romantic":
        events = get_death_reaction_dicts(family_relation, rel_value)
        possible_events.extend(events["general"][body_status])
        if trait in events and body_status in events[trait]:
            possible_events.extend(events[trait][body_status])
...
def possible_lead_den_events(
    ...
    events = get_lead_den_event_dicts(event_type, success)
    for event in events:
